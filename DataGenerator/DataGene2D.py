...
class DataGene2D:
    def __init__(self, device, n=10, m=10,
        ...
        self.x = np.repeat(self.x, self.y_range, axis=0)
        self.x = np.expand_dims(self.x, axis=0)
        self.x = np.repeat(self.x, IC, axis=0)
        self.x = np.expand_dims(self.x, axis=4)

        self.y = np.expand_dims(self.y, axis=1)
        self.y = np.repeat(self.y, self.x_range, axis=1)
        self.y = np.expand_dims(self.y, axis=0)
        self.y = np.repeat(self.y, IC, axis=0)
        self.y = np.expand_dims(self.y, axis=4)

        self.c = np.zeros((self.IC, self.n, self.m))

        for seed in range(1, self.IC + 1, 1):
            np.random.seed(seed)
            self.c[seed - 1] = np.random.normal(0.0, 1.0, (self.n, self.m))
    ...
    def Plain(self, dt):
        ...
        U = np.zeros((self.IC, self.x_range, self.y_range, t_range))

        t = np.repeat(t, self.x_range, axis=1)
        t = np.repeat(t, self.y_range, axis=2)

        x = np.repeat(self.x, t_range, axis=3)
        y = np.repeat(self.y, t_range, axis=3)

        for i in trange(self.n):
            for j in trange(self.m):
                temp_c = np.expand_dims(self.c[:, i, j], axis=1)
                temp_c = np.expand_dims(temp_c, axis=1)
                temp_c = np.expand_dims(temp_c, axis=4)
                temp_c = np.repeat(np.repeat(np.repeat(temp_c, self.x_range, axis=1), self.y_range, axis=2), t_range,
                                   axis=3)
                temp_exp = np.exp(t * -self.beta * ((np.pi * i / self.y_max) ** 2) + (np.pi * j / self.x_max) ** 2)
                temp_term = temp_c * np.sin((i * np.pi * y) / self.y_max) * np.sin((j * np.pi * x) / self.x_max)
                temp = temp_term * temp_exp
                U += (1.0 / (self.n * self.m)) * temp
    ...
    def Noise(self, dt, err_scale):
        ...
        for i in range(self.n):
            temp_c = \
                np.repeat(np.repeat(np.expand_dims(np.expand_dims(self.c[:, i], axis=1), axis=1), self.x_range, axis=1),
                          t_range, axis=2)
